...
//cant remove these yet, but builds ok.
...
//	return reward
	...
	// Epoch 2 - Beyond Block 4m
	if headerNumber.Cmp(big.NewInt(4000001)) > 0 {
		reward = new(big.Int).Mul(big.NewInt(5), big.NewInt(1e+18))
	}
	// Epoch 3 - Beyond Block 6m
	if headerNumber.Cmp(big.NewInt(6000001)) > 0 {
		reward = new(big.Int).Mul(big.NewInt(4), big.NewInt(1e+18))
	}
	// Epoch 4 - Beyond Block 8m
	if headerNumber.Cmp(big.NewInt(8000001)) > 0 {
		reward = new(big.Int).Mul(big.NewInt(3), big.NewInt(1e+18))
	}
